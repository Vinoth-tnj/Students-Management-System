...
$query->execute();
 echo "<script>alert('Data deleted');</script>"; 
  echo "<script>window.location.href = 'staff_manage-students.php'</script>";     
...
$sql="SELECT tblstudent.StuID,tblstudent.ID as sid,tblstudent.StudentName,tblstudent.StudentEmail,tblstudent.DateofAdmission,tblclass.ClassName,tblclass.Section,ts.staff_id from tblstaff ts,tblstudent join tblclass on tblclass.ID=tblstudent.StudentClass WHERE ts.class_handle=StudentClass AND ts.id=$aid LIMIT $offset, $no_of_records_per_page ";
$query = $dbh -> prepare($sql);
$query->execute();
...
{               ?>   
                          <tr>
                            ...
                            <td>
                              <div><a href="staff_edit-student-detail.php?editid=<?php echo htmlentities ($row->sid);?>"><i class="icon-eye"></i></a>
                                                || <a href="staff_manage-students.php?delid=<?php echo ($row->sid);?>" onclick="return confirm('Do you really want to Delete ?');"> <i class="icon-trash"></i></a></div>
                            </td> 
                          </tr><?php $cnt=$cnt+1;
                          if(true){
                            $stuid=$row->StuID;
                            $stucls=$row->ClassName;
                            $stusec=$row->Section;
                            $staffid=$row->staff_id;
                        $res = add_staff_stu($stuid,$stucls,$stusec,$staffid);}
                        }} ?>
                        </tbody>
                      </table>
                    </div>

...
$sqlQuery = "SELECT * FROM result WHERE semester=1";
...
foreach ($result as $row) {
	if($row['sub'] == "English Communication-1")
    {
        $count1_total++;
    }
    if($row['sub'] == "English Communication-1" && $row['marks']>=50)
    {
        $count1_pass++;
    }
    if($row['sub'] == "Matrices & Calculus")
    {
        $count2_total++;
    }
    if($row['sub'] == "Matrices & Calculus" && $row['marks']>=50)
    {
        $count2_pass++;
    }
    if($row['sub'] == "Financial Accounting")
    {
        $count3_total++;
    }
    if($row['sub'] == "Financial Accounting" && $row['marks']>=50)
    {
        $count3_pass++;
    }
    if($row['sub'] == "Basics of Computing")
    {
        $count4_total++;
    }
    if($row['sub'] == "Basics of Computing" && $row['marks']>=50)
    {
        $count4_pass++;
    }
    ...
    if($row['sub'] == "Office Automation Lab")
    {
        $count7_total++;
    }
    if($row['sub'] == "Office Automation Lab" && $row['marks']>=50)
    {
        $count7_pass++;
    }
    if($row['sub'] == "Ethics-1")
    ...
    if($row['sub'] == "Ethics-1" && $row['marks']>=50)
    {
        $count8_pass++;
    }
    if($row['sub'] == "Personality Development -1")
    {
        $count9_total++;
    }
    if($row['sub'] == "Personality Development -1" && $row['marks']>=50)
    {
        $count9_pass++;
    }
}
$datas = array();
$sub = array("English Communication-1","Matrices & Calculus","Financial Accounting","Basics of Computing","Programming in C","Programming in C Lab","Office Automation Lab","Ethics-1","Personality Development -1");
